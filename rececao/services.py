...
def real_ocr_extract(file_path: str):
    """
    Método inteligente de extração usando múltiplas estratégias.
    Suporta: PDF, imagens, Excel, CSV com OCR tradicional, tabelas e LLM.
    """
    print(f"🔍 Processando documento: {os.path.basename(file_path)}")
    
    # Usa o novo método inteligente que escolhe a melhor estratégia
    result = smart_ocr_extract(file_path)
    
    # Se smart_ocr_extract retornou resultado válido, usa-o
    if result:
        save_extraction_to_json(result)
        return result
    
    # Fallback para o método tradicional (não deve chegar aqui)
    text_content = ""
    qr_codes = []
    ext = os.path.splitext(file_path)[1].lower()

    print(f"⚠️ Fallback para método tradicional")
    ...
    if texto_pdfplumber_curto:
        print(f"⚠️ Texto muito curto ({len(text_content)} chars)")
...
# ======================== NOVOS MÉTODOS DE LEITURA ========================

def extract_from_excel(file_path: str):
    """
    Extrai dados de arquivo Excel (.xlsx, .xls).
    Procura automaticamente por tabelas de produtos.
    """
    import pandas as pd
    
    try:
        print(f"📊 Processando Excel: {os.path.basename(file_path)}")
        ...
        for sheet_name in xl_file.sheet_names:
            ...
            print(f"📄 Sheet: {sheet_name} ({len(df)} linhas)")
            
            # Normaliza cabeçalhos para strings (pode haver números ou NaN)
            df.columns = [str(col).strip() if col is not None and str(col) != 'nan' else f'col_{i}' 
                          for i, col in enumerate(df.columns)]
            
            # Tenta identificar colunas relevantes (case-insensitive)
            columns_lower = {str(col).lower(): col for col in df.columns if col}
            
            # Mapeia colunas comuns
            col_map = {}
            for key in ['artigo', 'código', 'codigo', 'ref', 'referencia', 'referência', 'sku']:
                if key in columns_lower:
                    col_map['artigo'] = columns_lower[key]
                    break
            
            for key in ['descrição', 'descricao', 'description', 'desc', 'produto']:
                if key in columns_lower:
                    col_map['descricao'] = columns_lower[key]
                    break
            
            for key in ['quantidade', 'qtd', 'qty', 'quant']:
                if key in columns_lower:
                    col_map['quantidade'] = columns_lower[key]
                    break
            
            for key in ['unidade', 'un', 'unit', 'und']:
                if key in columns_lower:
                    col_map['unidade'] = columns_lower[key]
                    break
            
            # Se encontrou pelo menos artigo e quantidade, processa
            if 'artigo' in col_map and 'quantidade' in col_map:
                for idx, row in df.iterrows():
                    try:
                        artigo = str(row[col_map['artigo']]).strip()
                        # Normaliza quantidade (pode vir como string ou número)
                        qty_raw = str(row[col_map['quantidade']]).strip()
                        # Remove pontos de milhar PT e substitui vírgula por ponto
                        if ',' in qty_raw:
                            qty_raw = qty_raw.replace('.', '').replace(',', '.')
                        quantidade = float(qty_raw) if qty_raw and qty_raw != 'nan' else 0
                        
                        if artigo and artigo.lower() not in ['nan', 'none', ''] and quantidade > 0:
                            produto = {
                                'artigo': artigo,
                                'descricao': str(row[col_map.get('descricao', col_map['artigo'])]).strip() if 'descricao' in col_map else '',
                                'quantidade': quantidade,
                                'unidade': str(row[col_map.get('unidade', 'UN')]).strip() if 'unidade' in col_map else 'UN',
                            }
        ...
        print(f"✅ Extraídos {len(all_products)} produtos do Excel")
        return {
            'produtos': all_products,
            'tipo_documento': 'EXCEL',
            'texto_completo': f"Excel com {len(all_products)} produtos",
            'totals': {
                'total_lines': len(all_products),
                'total_quantity': sum(p.get('quantidade', 0) for p in all_products)
            }
        }
...
def extract_from_csv(file_path: str):
    ...
    try:
        print(f"📄 Processando CSV: {os.path.basename(file_path)}")
        ...
        print(f"📊 CSV: {len(df)} linhas com delimitador '{delimiter}'")
        
        # Normaliza cabeçalhos para strings (pode haver números ou valores vazios)
        df.columns = [str(col).strip() if col is not None and str(col) != 'nan' else f'col_{i}' 
                      for i, col in enumerate(df.columns)]
        
        # Mapeia colunas (similar ao Excel)
        columns_lower = {str(col).lower(): col for col in df.columns if col}
        ...
        print(f"✅ Extraídos {len(all_products)} produtos do CSV")
        return {
            'produtos': all_products,
            'tipo_documento': 'CSV',
            'texto_completo': f"CSV com {len(all_products)} produtos",
            'totals': {
                'total_lines': len(all_products),
                'total_quantity': sum(p.get('quantidade', 0) for p in all_products)
            }
...
def extract_tables_with_camelot(file_path: str):
    """
    Extrai tabelas de PDF usando Camelot (melhor para PDFs com tabelas bem definidas).
    Fallback para detecção de OCR tradicional.
    """
    try:
        import camelot
        
        print(f"📊 Extraindo tabelas com Camelot: {os.path.basename(file_path)}")
        
        # Tenta extrair tabelas
        tables = camelot.read_pdf(file_path, pages='all', flavor='lattice')
        
        # Se lattice não funcionar, tenta stream
        if len(tables) == 0:
            print("⚠️ Lattice não encontrou tabelas, tentando Stream...")
        ...
        for table_num, table in enumerate(tables):
            df = table.df
            print(f"📄 Tabela {table_num + 1}: {len(df)} linhas, {len(df.columns)} colunas")
            
            # Tenta identificar header
            header_row = 0
            for idx, row in df.iterrows():
                row_str = ' '.join(str(cell).lower() for cell in row.values)
                if any(keyword in row_str for keyword in ['artigo', 'código', 'referência', 'quantidade', 'qtd']):
                    header_row = idx
            ...
            for key in ['descrição', 'descricao', 'description', 'desc']:
                if key in columns_lower:
                    col_map['descricao'] = columns_lower[key]
                    break
            ...
            for key in ['unidade', 'un', 'unit']:
                if key in columns_lower:
                    col_map['unidade'] = columns_lower[key]
                    break
            
            # Extrai produtos
            if 'artigo' in col_map:
                for idx, row in df.iterrows():
                    try:
                        artigo = str(row[col_map['artigo']]).strip()
                        quantidade_str = str(row[col_map.get('quantidade', '0')]).strip()
        ...
        if all_products:
            print(f"✅ Extraídos {len(all_products)} produtos com Camelot")
            return {
                'produtos': all_products,
                'tipo_documento': 'PDF_TABELAS',
                'texto_completo': f"PDF com {len(all_products)} produtos extraídos de tabelas",
                'totals': {
                    'total_lines': len(all_products),
                    'total_quantity': sum(p.get('quantidade', 0) for p in all_products)
                }
            ...
            print("⚠️ Camelot não encontrou produtos, usando OCR tradicional")
            return None
    
    except Exception as e:
        print(f"❌ Erro ao usar Camelot: {e}")
        return None


def extract_with_llm(text_content: str, image_path=None):
    """
    Usa LLM (OpenAI/Gemini) para extrair dados de documentos complexos.
    Fallback: retorna None se não houver API key configurada.
    """
    try:
        # Verifica se tem OpenAI API key
        openai_key = os.environ.get('OPENAI_API_KEY')
        gemini_key = os.environ.get('GEMINI_API_KEY')
        
        if not openai_key and not gemini_key:
            print("⚠️ Nenhuma API key configurada para LLM - pulando extração com IA")
            return None
        
        print("🤖 Usando LLM para extração de dados...")
        
        # Prepara prompt
        prompt = f"""Você é um especialista em extrair dados de documentos comerciais portugueses.

Analise o seguinte documento e extraia as informações em formato JSON:
...
{{
  "numero_documento": "número da guia/fatura",
...
- Use valores reais extraídos do texto
        ...
        if openai_key:
            try:
                ...
                response = client.chat.completions.create(
                    model="gpt-4o-mini",
                    messages=[
                        {"role": "system", "content": "Você é um especialista em extração de dados de documentos comerciais. Responda sempre em JSON válido."},
                ...
                if content:
                    ...
                    print("✅ Dados extraídos com OpenAI")
            except Exception as e:
                print(f"⚠️ Erro ao usar OpenAI: {e}")
        ...
        if not result and gemini_key:
            try:
                ...
                if response.text:
                    ...
                    print("✅ Dados extraídos com Gemini")
            except Exception as e:
                print(f"⚠️ Erro ao usar Gemini: {e}")
        
        if result and 'produtos' in result:
            produtos = result.get('produtos', [])
            return {
                'produtos': produtos,
                'document_number': result.get('numero_documento', ''),
                'supplier_name': result.get('fornecedor', ''),
                'delivery_date': result.get('data', ''),
                'tipo_documento': 'LLM_EXTRACTED',
                'texto_completo': text_content,
                'totals': {
                    'total_lines': len(produtos),
                    'total_quantity': sum(p.get('quantidade', 0) for p in produtos)
                }
            }
...
def smart_ocr_extract(file_path: str):
    """
    Método inteligente que escolhe a melhor estratégia de extração baseado no tipo de arquivo.
    Ordem de tentativas:
    1. Se Excel/CSV: usa parser específico
    2. Se PDF com tabelas: tenta Camelot
    3. Se tem LLM configurado: usa LLM
    4. Fallback: OCR tradicional
    """
    ext = os.path.splitext(file_path)[1].lower()
    
    # 1. Excel/CSV - extração direta
    if ext in ['.xlsx', '.xls']:
    ...
    # 2. PDF - tenta Camelot primeiro para tabelas
    elif ext == '.pdf':
        camelot_result = extract_tables_with_camelot(file_path)
        if camelot_result:
    ...
    # 4. Se OCR falhou ou texto muito curto, tenta LLM
    if len(text_content) < 100:
        llm_result = extract_with_llm(text_content, file_path)
        if llm_result:
            return llm_result
